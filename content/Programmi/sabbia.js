...
function draw() {
  ...
  F.update()
  if (checkbox1.checked()) F.draw_force()
...
class forceField {
  constructor(n, m) {
    ...
    this.positions = Array.from({ length: m }, (v, i) =>
      Array.from({ length: n }, (w, j) => p5.Vector.mult(this.offset, [j + 0.5, i + 0.5])),
    )
  }
    ...
    for (i = 0; i < this.m; i++) {
      for (j = 0; j < this.n; j++) {
        this.force[i][j] = p5.Vector.fromAngle(
          noise(j * this.resolution.x, i * this.resolution.y, millis() * 0.0001) * TWO_PI * 1.5,
        )
